...
void Server::ProcessRdmaRequest(ibv_wc& wc) {
  ...
  switch (wc.opcode) {
    ...
    case IBV_WC_RDMA_WRITE:
      //update pending_msg
      epicLog(LOG_DEBUG, "get write completion event to worker node %d", cli->GetWorkerId());
      id = cli->WriteComp(wc);
    ...
    do {
      ...
      for (int i = 0; i < ne; ++i) {
        ...
        ProcessRdmaRequest(wc[i]);
          ...
          epicLog(LOG_WARNING," RDMA request processing from Node: %d to Node %d part 1  duration is %ld ns", qpCliMap[wc[i].qp_num], GetWorkerId(),  duration.count());
#endif
      }
