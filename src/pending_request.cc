...
void Worker::ProcessPendingWrite(Client* cli, WorkRequest* wr) {
  ...
  if (wr->counter == 0 || (wr->flag & REQUEST_DONE)) {
    ...
    parent->unlock();
      if (parent->op == WLOCK){
          epicLog(LOG_INFO, "PENDING WLOCK is under execution and the notify signal is %d", notify);
      }
    if (notify)
      Notify(parent);
...
//    epicLog(LOG_WARNING, "Process pending cache eviction");
  int ret = ErasePendingWork(wr->id);
  epicAssert(ret);
  epicLog(LOG_INFO, "Cache entry eviction reply for %llu has been processed", wr->addr);
  ProcessToServeRequest(wr);
  delete wr;
  wr = nullptr;
}
...
void Worker::ProcessPendingInvalidateForward(Client* cli, WorkRequest* wr) {
  ...
  if (wr->counter == 0) {
    ...
    if (WRITE == parent->op) {
      ...
      epicLog(LOG_INFO, "write the data (size = %ld) to destination",
              parent->size);
