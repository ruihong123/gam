...
//      epicLog(LOG_WARNING, "Cach miss, Remote operation %d, local buffer is %p, remote buffer is %p, size is %d, target node is %d\n", lwr->op, lwr->ptr, lwr->addr, lwr->size, cli->GetWorkerId());
      worker->SubmitRequest(cli, lwr, ADD_TO_PENDING | REQUEST_SEND);
    }
    unlock(i);
    i = nextb;
...
//  UnLinkLRU(cline, pos); //since we already got the lock in the parent function of Evict(CacheLine*)
    ...
    epicLog(LOG_WARNING, "Cache entry for %d is being evicted", wr->addr);
  } else {  //invalid
