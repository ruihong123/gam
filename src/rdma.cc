  ...
  if (op == IBV_WR_SEND) {
    ...
    sge_list.lkey = send_buf->lkey;
      ...
      if (op == IBV_WR_RDMA_WRITE_WITH_IMM){
          epicLog(LOG_WARNING,"RDMA write source is %p, destination is %p, len is%d, id is %llu \n", src, dest, len, wr.wr_id);

      }
    sge_list.addr = (uintptr_t) src;
    sge_list.lkey = resource->bmr->lkey;
    wr.wr.rdma.remote_addr = (uintptr_t) dest;
//    if (((uint64_t )dest & (0x1ull <<63)) == 0)
//        epicLog(LOG_WARNING, "RDMA write destination is %p, %llu", dest,dest);
    assert((uint64_t)dest != 0 || (op == IBV_WR_RDMA_WRITE_WITH_IMM && len == 0));
    // It is possible to have 0 dest if the command is write with imm
