...
#    ssh -o StrictHostKeyChecking=no ${compute_shard[n]}  "screen -d -m pwd && cd /users/Ruihong && git clone --recurse-submodules $github_repo && cd MemoryEngine/ && mkdir release &&  cd release && cmake -DCMAKE_BUILD_TYPE=Release .. && sudo apt install numactl -y && sudo apt-get install -y libnuma-dev htop  && make micro_bench memory_server_term -j" &
...
#    ssh -o StrictHostKeyChecking=no ${memory_shard[n]}  "screen -d -m pwd && cd /users/Ruihong && git clone --recurse-submodules $github_repo && cd MemoryEngine/ && mkdir release &&  cd release && cmake -DCMAKE_BUILD_TYPE=Release .. && sudo apt install numactl -y && sudo apt-get install -y libnuma-dev htop  && make micro_bench memory_server_term -j" &
...
do
  ...
  ssh -o StrictHostKeyChecking=no $node "pkill -f benchmark  && pkill -f memory_server && cd $BIN_HOME" #> /dev/null 2>&1
done
for node in ${compute_shard[@]}
do
  echo "kill the zombie process on $node"
  ssh -o StrictHostKeyChecking=no $node "pkill -f memory_server && pkill -f benchmark && cd $BIN_HOME"
done
for node in ${memory_shard[@]}
do
  echo "Rsync the $node"
...
#      rsync -a ~/gam ${compute_shard[n]}:~/gam
#      echo "Set up the ${memory_shard[n]}"
#      rsync -a ~/gam ${compute_shard[n]}:~/gam
#      n=$((n+1))
#      sleep 1
