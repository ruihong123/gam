...
do
  ...
  ssh -o StrictHostKeyChecking=no $node "pkill -f benchmark  && pkill -f memory_server" #> /dev/null 2>&1
done
for node in ${compute_shard[@]}
do
  echo "kill the zombie process on $node"
  ssh -o StrictHostKeyChecking=no $node "pkill -f memory_server && pkill -f benchmark"
done
for node in ${memory_shard[@]}
do
  echo "Rsync the $node"
