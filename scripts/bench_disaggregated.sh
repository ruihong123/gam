...
log_file=$bin/log
cache_mem_size=1 # 10 gb Local memory size
...
#    	fi
    ...
    for memory in `cat "$memory_nodes"`
        do
        	...
        	echo "$SRC_HOME/memory_server --op_type $op_type  --no_thread $thread --shared_ratio $shared_ratio --read_ratio $read_ratio --space_locality $space_locality --time_locality $time_locality --result_file $result_file --ip_master $master_ip --ip_worker $ip --port_worker $port --port_master $master_port --compute_num $compute_num --memory_num $memory_num" | tee -a "$log_file".$ip
